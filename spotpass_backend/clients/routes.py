...
def list_clients(
    ...
    valid_labels = ["vip", "loyal", "blacklisted"]
    if label_filter:
    ...
    # Apply label filter - each flag is independent
    if label_filter:
        conditions = []
        for label in label_filter:
            if label == "vip":
                conditions.append(col(Client.is_vip).is_(True))
            elif label == "loyal":
                conditions.append(col(Client.is_loyal).is_(True))
            elif label == "blacklisted":
                conditions.append(col(Client.is_blacklisted).is_(True))
        if conditions:
    ...
    if search:
        ...
        statement = statement.where(
            or_(
                col(Client.full_name).ilike(search_term),
                col(Client.phone_number).ilike(search_term),
                col(Client.email).ilike(search_term),
            )
        )
    ...
    if sort_by == "name":
        ...
        # Sort by multiple status flags independently
        # VIP clients first, then loyal, then blacklisted, then regular (no flags)
        from sqlalchemy import case

        # Create composite score: VIP=100, Loyal=10, Blacklisted=-100
        # This allows sorting by priority: VIP+Loyal (110) > VIP (100) > Loyal (10) > Regular (0) > Blacklisted (-100)
        status_order = (
            case((col(Client.is_vip).is_(True), 100), else_=0)
            + case((col(Client.is_loyal).is_(True), 10), else_=0)
            + case((col(Client.is_blacklisted).is_(True), -100), else_=0)
        )
...
def create_client(client_data: ClientCreate, session: DatabaseSession, token_payload: StaffUser):
    ...
    if establishment.id is None:
        raise HTTPException(
            ...
            detail="Establishment ID is not set",
        )
    ...
    if existing_client:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Un client avec le même numéro de téléphone existe déjà",
        )

    # Check for duplicate email (if provided)
    if client_data.email:
        statement = select(Client).where(
            Client.email == client_data.email, Client.establishment_id == establishment.id
        )
        existing_client_email = session.exec(statement).first()

        if existing_client_email:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Un client avec la même adresse email existe déjà",
            )
